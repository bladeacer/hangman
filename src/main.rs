...
impl App {
    ...
    fn draw(&self, frame: &mut Frame) {
        ...
        let [greeting_rect, _test] = Layout::horizontal([Constraint::Fill(1), Constraint::Length(29)]).areas(top);
    ...
    fn render_rand_text(&self, frame: &mut Frame, area: Rect) {
        ...
        let replaced_string: String = rand_word.chars().map(|_| replacement_symbol).collect();
        let mut greeting_text = String::from("Hello World (pres 'q' to exit)\nRandom string: ");
        greeting_text.push_str(rand_word);
        greeting_text.push_str("\nReplaced String: ");
        greeting_text.push_str(&replaced_string);
